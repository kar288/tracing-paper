...
CWDatastore._syncStoreToDevice = function(target) {
  console.log('_syncStoreToDevice---------------------------------------------------------------');
...
CWTemplates.insert = function(templateName, target, data, callback) {
  ...
  $.when.all(this._files).always(function() {
    ...
    ractive.on('test', function(event, id, foo) {
      debugger;
    });
...
CWStitchManager.detectedSwipe = function(data) {
  ...
  console.log('Detected swipe on ' + swipe.device + ' on edge ' + swipe.edge );
  ...
  for (var key in this._swipes) {
    ...
    console.log('Checking existing swipe: '+key);
    ...
    console.log('Checking time constraint');
    ...
    console.log('Checking connection constraint');
